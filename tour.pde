...
class Tour {
  ...
  *****************************************************/
  ...
  void makeNearestNeighbourTourPlan(){
    ...
    tourLength = getDistance(tourPlan[0], tourPlan[numCities-1]);
  ...
  void makeRandNearestNeighbourTourPlan(float randProbability){
    ...
    for (int i=1; i< numCities; i++){
	  ...
	  tourPlan[i] = unvisited.get((int)random(unvisited.size())); // fill in
      }
      else{
	  tourPlan[i] = getNearestNeighbour(currentLocation, unvisited); // fill in
      }
